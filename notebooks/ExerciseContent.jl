...
const EXERCISES = Exercise[
    ...
    Exercise(
        number=2,
        title="Read a CP decomposition",
        notebook="notebooks/00_Primer.jl",
        location="Section 3",
        minutes=7,
        introduction="Fit the rank-2 CP model to the running order-three tensor and inspect the two linked outer-product patterns.",
        sections=[ExerciseSection(questions=[
            q("1", "How many rank-one components are added in the fitted model?", "Two rank-one components."; options=["one", "two", "three"]),
            q("2", "How many mode vectors are linked inside each component of this order-three tensor?", "Three vectors—one from each mode."; options=["one", "two", "three"]),
            q("3", "Complete one component: uᵣ⁽¹⁾ ⊗ ____ ⊗ ____.", "uᵣ⁽¹⁾ ⊗ uᵣ⁽²⁾ ⊗ uᵣ⁽³⁾."),
            q("4", "What does the shared component index r do across the three factor matrices?", "It links one column from every mode into one complete outer-product pattern."; answer_lines=2),
            q("5", "Record the relative reconstruction error displayed for your running tensor.", "Accept the finite relative error shown in the CP component inspector."),
            q("6", "What does that reconstruction error establish?", "It measures how closely these two fitted rank-one components reconstruct the target tensor."; answer_lines=2),
            q("7", "Does a small reconstruction error prove that the two components are unique or semantic concepts?", "No. Fit alone does not establish uniqueness, stability, or semantic meaning."; options=["Yes", "No"]),
        ])],
    ),
    Exercise(
        number=3,
