...
const RAW_MATH_GLOSSARY = [
    (term="Tensor", category="Mathematics", definition="In this tutorial, a tensor is represented by a multidimensional array \$\\mathcal X\\in\\mathbb R^{n_1\\times\\cdots\\times n_d}\$. More abstractly, a tensor is an element of a tensor-product space; the array contains its coordinates in chosen bases.", tutorial="The notebooks preserve meaningful modes instead of always flattening them.", reading="Kolda & Bader (2009)", related=["Mode", "Matricization", "CP decomposition", "Tucker decomposition"]),
    ...
    (term="Rank-one tensor", category="Mathematics", definition="A tensor that can be written as one outer product \$a^{(1)}\\otimes\\cdots\\otimes a^{(d)}\$.", tutorial="It is the basic component of CP decomposition and the Segre model.", reading="Kolda & Bader (2009); Breiding & Vannieuwenhoven (2018)", related=["Outer product", "Rank-one / Segre geometry", "CP decomposition"]),
    ...
    (term="HOSVD", category="Mathematics", definition="Higher-Order SVD constructs mode-wise orthonormal bases from SVDs of tensor unfoldings.", tutorial="A truncated HOSVD gives a controlled Tucker approximation, but is not generally the globally best approximation of a prescribed multilinear rank.", reading="De Lathauwer, De Moor & Vandewalle (2000)", related=["Tucker decomposition", "Multilinear rank", "SVD"]),
    ...
    (term="SVD", category="Mathematics", definition="For a matrix \$X\$, \$X=U\\Sigma V^\\top\$. Truncating to the \$k\$ largest singular values gives a best rank-\$k\$ approximation in Frobenius and spectral norm.", tutorial="Applied to a chosen matricization, it can still merge mode semantics after flattening.", reading="Standard linear algebra; Kolda & Bader (2009)", related=["Matrix rank", "Matricization / unfolding", "Subspace"]),
    ...
    (term="Rank-one / Segre geometry", category="Mathematics", definition="Nonzero rank-one tensors form a smooth manifold. After quotienting by overall scaling, their projective classes form the Segre variety.", tutorial="The affine cone over the Segre variety also contains the zero tensor; a CP tensor is a sum of nonzero Segre components.", reading="Breiding & Vannieuwenhoven (2018)", related=["Rank-one tensor", "CP decomposition", "Riemannian optimization"]),
    (term="Fixed-rank manifold", category="Mathematics", definition="The set \$\\{X\\in\\mathbb R^{m\\times n}:\\operatorname{rank}(X)=r\\}\$ is a smooth embedded manifold.", tutorial="Matrices of smaller rank lie in the boundary of its closure, not in the fixed-rank manifold itself.", reading="Absil, Mahony & Sepulchre (2008); Bian et al. (2025)", related=["Matrix rank", "Factorized parameterization", "Parameter redundancy / overparameterization"]),
    ...
    (term="Identifiability", category="Mathematics", definition="A parameterized model is identifiable at an object when its fiber contains only parameter values related by the accepted model symmetries.", tutorial="Local, global, and generic identifiability are distinct. Identifiability supports coordinate recovery, but does not by itself establish scientific meaning.", reading="Kolda & Bader (2009); De Lathauwer (2008)", related=["Coordinates vs represented object", "Semantic validation", "Stability"]),
    (term="Conditioning", category="Mathematics", definition="The local sensitivity of a specified recovery or solution map to input perturbations, relative to chosen norms or metrics and after known representation symmetries are handled.", tutorial="A low reconstruction error can coexist with unstable component coordinates; always ask which map and metric a condition number describes.", reading="Breiding & Vannieuwenhoven (2018)", related=["Condition number", "Component collision", "Stability"]),
    ...
    (term="Excessive latent rank / over-ranked factorization", category="Mathematics", definition="Choosing more latent components or a larger rank than the structure being modeled requires.", tutorial="Extra components can split, duplicate, or collide with structure and complicate interpretation; this is narrower than general overparameterization.", reading="General low-rank modeling background", related=["Rank choice", "Feature splitting", "Component collision", "Parameter redundancy / overparameterization"]),
    (term="Rank choice", category="Mathematics", definition="The selected number of components or latent dimensions used by a model.", tutorial="Rank controls both approximation capacity and the granularity of the learned vocabulary.", reading="Labs 2 and 4", related=["Excessive latent rank / over-ranked factorization", "Concept granularity", "Model mismatch"]),
    (term="ALS", category="Mathematics", definition="Alternating Least Squares updates one factor block at a time while keeping the others fixed.", tutorial="It is simple and effective, but its local least-squares systems can become poorly conditioned.", reading="Kolda & Bader (2009)", related=["Condition number", "Randomized ALS (RALSSolver in TensorKitchen)", "Regularized ALS", "Swamp / swamp-like stagnation"]),
    (term="Randomized ALS (RALSSolver in TensorKitchen)", category="Mathematics", definition="Randomized Alternating Least Squares approximates ALS block updates using sampled rows of the Khatri-Rao least-squares problem.", tutorial="The acronym RALS is overloaded in the literature; here it refers specifically to TensorKitchen v0.2.0's randomized CP-ALS solver (CPRAND/CPRAND-MIX), not the ridge-regularized teaching solver in Lab 3.", reading="TensorKitchen documentation; tensor optimization literature", related=["ALS", "Regularized ALS"]),
    (term="Regularized ALS", category="Mathematics", definition="Regularized ALS adds a penalty or stabilizing term, such as a ridge term, to alternating least-squares subproblems.", tutorial="Lab 3 uses a small ridge-regularized ALS implementation to study conditioning. It is distinct from TensorKitchen's randomized `RALSSolver`.", reading="Tensor optimization literature; Lab 3", related=["ALS", "Randomized ALS (RALSSolver in TensorKitchen)", "Conditioning"]),
    ...
    (term="Retraction", category="Mathematics", definition="A smooth map \$R_x:T_x\\mathcal M\\to\\mathcal M\$ satisfying \$R_x(0)=x\$ and \$D R_x(0)=\\mathrm{id}_{T_x\\mathcal M}\$.", tutorial="It provides a first-order approximation to moving along the manifold and turns a tangent search direction into a feasible point.", reading="Absil, Mahony & Sepulchre (2008)", related=["Tangent space", "RGD", "RCG"]),
    ...
    (term="Degeneracy", category="Mathematics", definition="In CP approximation, a degenerating sequence can contain rank-one terms whose norms diverge while their sum remains bounded or converges.", tutorial="Near-cancellation is a characteristic mechanism, but not every finite cancellation example is itself a degenerating sequence.", reading="Kolda & Bader (2009); Breiding & Vannieuwenhoven (2018)", related=["Cancellation", "Component collision", "Conditioning"]),
...
const RAW_AI_GLOSSARY = [
    ...
    (term="Monosemanticity", category="AI & Interpretability", definition="The ideal that one learned feature corresponds to one coherent semantic pattern rather than several unrelated meanings.", tutorial="SAE work investigates whether sparse dictionaries yield more monosemantic features than individual neurons.", reading="Bricken et al. (2023); Cunningham et al. (2023); Templeton et al. (2024; arXiv 2026)", related=["Polysemanticity", "Semantic coherence", "Sparse autoencoder"]),
    ...
    (term="Concept importance", category="AI & Interpretability", definition="A measure of how model output varies when a candidate concept coordinate is varied under a specified perturbation or sampling scheme.", tutorial="CRAFT estimates importance with Sobol sensitivity rather than coefficient magnitude alone. This sensitivity does not by itself establish causal relevance in the underlying data-generating process.", reading="Fel et al. (2023)", related=["Concept usage / coefficient", "Sobol sensitivity", "Causal relevance", "Intervention"]),
    ...
    (term="Adapter", category="AI & Interpretability", definition="A small trainable module inserted into or associated with a pretrained model for parameter-efficient adaptation.", tutorial="LoRA is a parameter-efficient adaptation method based on low-rank weight updates; terminology that reserves ‘adapter’ for an added module can therefore distinguish the two.", reading="Hu et al. (2022)", related=["LoRA", "Low-rank weight update"]),
    ...
    (term="Factorized parameterization", category="AI & Interpretability", definition="Representing a low-rank object through factors, e.g. \$W=AB^\\top\$.", tutorial="It reduces parameter count but introduces non-unique coordinates and can affect conditioning.", reading="Bian et al. (2025)", related=["Parameter redundancy / overparameterization", "Fixed-rank weight", "Gauge freedom / equivalence"]),
    (term="Parameter redundancy / overparameterization", category="AI & Interpretability", definition="A model has more parameter coordinates than are needed to specify its represented object or behavior, so different parameter values may encode the same object.", tutorial="This broad notion is distinct from choosing excessive tensor rank. RAdaGrad/RAdamW motivate direct fixed-rank optimization partly to avoid redundant factor coordinates.", reading="Bian et al. (2025)", related=["Factorized parameterization", "Coordinates vs represented object", "Fixed-rank manifold", "Excessive latent rank / over-ranked factorization"]),
    ...
    (term="Equivariance", category="AI & Interpretability", definition="A property where transforming the input induces a predictable corresponding transformation of the representation or output.", tutorial="Tensor Decomposition Networks study low-rank approximations inside SO(3)-equivariant architectures.", reading="Lin et al. (2025)", related=["Tensor product", "Clebsch–Gordan coupling / tensor-product decomposition", "Low-rank tensor operator"]),
    (term="Tensor product", category="AI & Interpretability", definition="A multilinear construction that combines representation spaces or features.", tutorial="Equivariant networks may use structured tensor-product interactions that can be expensive.", reading="Lin et al. (2025)", related=["Equivariance", "Clebsch–Gordan coupling / tensor-product decomposition"]),
    (term="Clebsch–Gordan coupling / tensor-product decomposition", category="AI & Interpretability", definition="The representation-theoretic coupling and decomposition used to combine SO(3)-equivariant features while respecting symmetry.", tutorial="The TDN paper calls the implemented operation a ‘CG tensor product’ and replaces expensive CG tensor-product operators with low-rank tensor decompositions.", reading="Lin et al. (2025)", related=["Equivariance", "Low-rank tensor operator", "Architectural compression"]),
...
const RAW_REFERENCES = [(key="Kolda & Bader (2009)", citation="T. G. Kolda and B. W. Bader. Tensor Decompositions and Applications. SIAM Review 51(3):455–500, 2009. DOI 10.1137/07070111X."),
    ...
    (key="Templeton et al. (2024; arXiv 2026)", citation="A. Templeton et al. Scaling Monosemanticity: Extracting Interpretable Features from Claude 3 Sonnet. Anthropic research publication, 2024; arXiv:2605.29358 version, 2026."),
...
const MATH_CLUSTER_TERMS = [
    ...
    "III. Geometry and equivalence" => Set([
        ...
        "Rank-one / Segre geometry", "Fixed-rank manifold",
        "Coordinates vs represented object", "Gauge freedom / equivalence",
        "Scaling ambiguity", "Permutation ambiguity", "Identifiability",
    ]),
    "IV. Optimization" => Set([
        "Nonconvex optimization", "Initialization", "ALS", "Randomized ALS (RALSSolver in TensorKitchen)",
        "Regularized ALS",
    ...
    "V. Failure and stability" => Set([
        ...
        "Excessive latent rank / over-ranked factorization", "Rank choice",
...
const AI_CLUSTER_TERMS = [
    ...
    "IV. Low-rank learning in neural networks" => Set([
        ...
        "Factorized parameterization", "Parameter redundancy / overparameterization",
        "Fixed-rank weight", "RAdaGrad / RAdamW", "Equivariance",
        "Tensor product", "Clebsch–Gordan coupling / tensor-product decomposition",
...
const REFERENCE_KEYS = Dict(
    ...
    "Templeton et al. (2024; arXiv 2026)" => "TEM26",
    "Li et al. (2025)" => "LIS25",
